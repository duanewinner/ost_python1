...
def histogram_function(count, y_scale=100):
    ...
    for wlen in count:
        if wlen > 64:
            ...
            if count[wlen] % y_tic == 0:  
                for i in range(count[wlen]): 
                    bins[count[wlen]].append(wlen)
            else:
                for i in range(count[wlen]):
                    bins[int(count[wlen] / y_tic + 1) * y_tic].append(wlen)
...
if __name__ == "__main__":
    ...
    print(histogram_function(count))
    ...
    while True:
        ...
        try:
            ...
            continue
        print(histogram_function(count, int(new_y_scale)))
